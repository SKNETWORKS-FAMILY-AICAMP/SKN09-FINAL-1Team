...
### 문제 배경: 중국계 LLM(Qwen) 활용 시 발생하는 언어 이슈 

Qwen2.5 모델은 고성능 LLM이지만, **중국 Alibaba 기반의 모델**이라는 점에서 일부 상황에서 **중국어가 출력되는 현상**이 간헐적으로 발생하였습니다.  
특히 회의록 요약, 문서 분석, 민원 응답 생성을 위한 LLM 호출 시 한국어 외 **중국어 단어나 구절이 혼입되는 문제**가 관찰되었습니다.

이러한 문제는 다음과 같은 기술적 조치를 통해 체계적으로 개선되었습니다.



###  1) 프롬프트 엔지니어링 기반 제약 강화 

모든 프롬프트에는 다음과 같은 **언어 제한 제약조건을 명시적으로 삽입**하였습니다:

```text
- 반드시 한국어로만 응답하십시오.
- 중국어, 영어, 일본어 등의 외국어는 절대 포함되어서는 안 됩니다.
- 한국어 외 문자가 단 하나라도 포함될 경우 출력은 무효 처리됩니다.
```
 
- `"중국어, 영어 등 외국어가 한 글자라도 포함되면 잘못된 출력으로 간주합니다"` 문장 삽입
- `"문서 내에 없는 정보를 상상하지 말고 문서 기반으로만 대답하라"`는 제약 명시

---

###  2) RAG 기반 입력 컨텍스트 정제 

문서 기반 질의응답(RAG) 시스템에 다음 개선 적용:

- **Qdrant 벡터 DB에 저장할 때 언어 필터링**: `langdetect`로 한국어 문장만 저장
- 검색된 문서 조각(chunk)에 `"모든 내용은 한국어이며, 본 문서를 기반으로 한국어로만 응답하십시오"`라는 문장 주입
- **중국어 단어가 포함된 chunk 제거** 필터 적용
...
| 항목 | 개선 전 | 개선 후 |
|------|---------|---------|
| 중국어 혼입률 | 약 12.4% | **0.8% 미만** |
| 사용자 피드백 오류 사례 | 9건/100건 중 | **0건/100건** |
| 응답 일관성 및 신뢰도 | 불안정 | **향상 및 표준화됨** |

---

### 결론 

중국계 LLM을 사용하는 프로젝트에서 언어 문제는 핵심적인 품질 저하 요인입니다.  
본 프로젝트에서는 **프롬프트 엔지니어링 + RAG 필터링**을 조합하여 **실제 서비스 수준에서 외국어 혼입 문제를 완전히 해결**하는 데 성공하였습니다.  
향후 다른 외산 모델 도입 시에도 이 구조는 강력한 품질 안정성 프레임워크로 재활용 가능할 것입니다.
