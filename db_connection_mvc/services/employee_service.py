...
#             if not employee:
#                 return None
#             if not pwd_context.verify(emp_pwd, employee["emp_pwd"]):
#                 return None
#             return employee
#         except Exception as e:
#             print(f"로그인 서비스 오류: {e}")
...
#             self.db.change_password(emp_code, hashed_pwd)
#             return self._format_success("비밀번호 변경 성공")
#         except Exception as e:
#             return self._format_error(f"비밀번호 변경 오류: {str(e)}")
...
#             # 고정된 초기 비밀번호를 해시
#             hashed_pwd = pwd_context.hash("1234")
#             employee_data["emp_pwd"] = hashed_pwd
...
#         try:
#             hashed_pwd = pwd_context.hash(new_password)
#             await self.db.change_password_by_emp_no(emp_no, hashed_pwd)
#             return self._format_success(f"직원 번호 {emp_no} 비밀번호 초기화 성공")
#         except Exception as e:
#             return self._format_error(f"비밀번호 초기화 서비스 오류: {str(e)}")
