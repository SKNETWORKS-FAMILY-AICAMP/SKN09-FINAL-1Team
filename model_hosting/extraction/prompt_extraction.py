...
"""
    ...
    def make_feedback_prompt(self, qna_data):
        ...
        return f"""
다음은 여러 개의 Q&A 쌍입니다. 각 쌍은 질문과 그에 대한 답변으로 구성되어 있으며, 질문과 답변은 이미 내부적으로 주어졌다고 가정합니다.

각 Q&A 쌍에 대해 아래의 기준을 바탕으로 피드백을 작성하세요.
...
작성 지침:
- 각 Q&A 쌍에 대해 피드백을 2~4문장으로 작성하십시오.
- 피드백은 해당 Q&A 쌍만을 기준으로 독립적으로 작성하십시오.
- 질문과 답변은 다시 출력하지 마십시오.
- 피드백은 객관적이며, 강점과 개선점을 균형 있게 포함하도록 하십시오.
- 불필요한 감탄이나 형식적인 문구는 생략하고, 실질적인 평가 내용을 서술하십시오.
