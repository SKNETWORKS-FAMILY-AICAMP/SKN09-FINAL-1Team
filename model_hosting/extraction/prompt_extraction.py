...
"""
    ...
    def make_feedback_prompt(self, qna_data):
        ...
        return f"""
다음은 하나의 질문과 그에 대한 답변으로 구성된 Q&A입니다.

{qna_text}

── 작업 지침 ──  
다음의 5가지 항목을 종합적으로 고려하여, 이 Q&A 쌍에 대한 피드백을 **충분히 자세하게** 서술해 주세요.

평가 기준:  
1. 답변이 질문의 맥락과 의도를 올바르게 이해하고 반영했는가?  
2. 법률적 규제나 정책을 위반하거나 부정확한 내용은 없는가?  
3. 회사의 서비스 방침이나 지침과 일치하는가?  
4. 질문자에 대한 태도와 언어 사용이 정중하고 적절한가?  
5. 전체적인 구성과 전달 방식이 명확하고 효과적인가?

출력 지침:
- 문장 수 제한은 없으나, 불필요하게 장황하지 않게 서술하세요.
- 강점과 개선점을 모두 언급해 주세요.
- 평가는 객관적이고 구체적으로, 사용자가 개선 방향을 이해할 수 있도록 작성하세요.

── 출력 예시 ──  
- 질문의 의도는 잘 파악되었고, 답변도 정중하게 요점을 설명하고 있어 전반적으로 적절합니다.  
- 다만 법률 관련 내용이 다소 추상적으로 표현되어 있어, 정확한 조항이나 사례 제시가 보완되면 더욱 신뢰를 줄 수 있습니다.  
- 회사 정책에도 부합하며, 고객을 배려한 어조 역시 긍정적으로 평가됩니다.
"""